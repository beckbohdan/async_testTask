...
async def parse_announcement(session, announcement_url) -> Announcement:
    html = await fetch_page(session, announcement_url)
...
async def parse_announcements(url, limit=60) -> List[Announcement]:
    ...
    # Create a custom SSL context to ignore SSL errors (e.g., SSL certificate verification)
        ...
        while len(announcements) < total_announcements:
            ...
            try:
                ...
                for link in links:
                    ...
                    task = parse_announcement(session, announcement_url)
                    tasks.append(task)

                results = await asyncio.gather(*tasks)
                announcements.extend(results)
                parsed_announcements += len(results)
...
if __name__ == "__main__":
    ...
    # Parse up to 60 announcements asynchronously
    announcements = asyncio.run(parse_announcements(url, limit=60))

    if announcements:
        # Save the announcements as JSON
        with open("announcements_async.json", "w") as outfile:
            json.dump(announcements, outfile, indent=2)
        print(f"Successfully fetched and saved {len(announcements)} announcements to announcements_async.json.")
    else:
